...
from .config import Config, generation_aspect

_OPENROUTER_URL = "https://openrouter.ai/api/v1/chat/completions"
_RETRYABLE_STATUS = {408, 409, 429, 500, 502, 503, 504, 524}

# 镜头组时长约束(秒):一个镜头组一次连续生成,总长范围随引擎
# (config.ENGINES 的 group_seconds);组内单个分镜最短可到 1 秒
_MIN_CUT_SECONDS = 1
...
def _clamp_duration(value, fallback: int, minimum: int, maximum: int) -> int:
    """把模型给出的时长钳制到 [minimum, maximum];缺失/非法时用回退值(同样钳制)。"""
    try:
        value = int(value)
...
class Cut:
    """镜头组内的一个分镜。"""
    prompt: str         # 交给视频模型的分镜提示词(语言随引擎)
    duration: int       # 秒,1 起,组内之和受引擎单次生成范围约束


@dataclass
class Shot:
    """一个镜头组:视频模型一次调用连续生成组内全部分镜,组内衔接由模型原生保证。"""
    index: int
    ...
    def from_dict(cls, data: dict) -> "Shot":
        ...
        if "cuts" in data:
            cuts = [
                Cut(prompt=str(c["prompt"]), duration=int(c.get("duration", _LEGACY_CUT_SECONDS)))
                for c in data["cuts"]
            ]
        else:
            cuts = [Cut(
                prompt=str(data["prompt"]),
                duration=int(data.get("duration", _LEGACY_CUT_SECONDS)),
            )]
        return cls(
...
_STORYBOARD_SCHEMA = {
    ...
    "properties": {
        ...
        "shots": {
            "type": "array",
            "description": (
                "Shot groups, played in order. Each group is generated by the video "
                "model in ONE continuous pass (native continuity inside the group); "
...
class Director:
    ...
    # 生成阶段送入视频模型的张数上限见 config.ENGINES 的 max_reference_images)
    ...
    def _write_once(
        ...
        min_group, max_group = spec.group_seconds
        ...
        if generation_aspect(engine, aspect) != aspect:  # 经裁剪实现,构图提示不同
            note = self._CROP_NOTES.get(aspect, note)
        if note:
            user_message += f"\n\n{note}"
        ...
        if resp.status_code == 400 and "reasoning" in body and _mentions_effort(resp):
            # 个别模型/提供方只认部分思考档位(如 GLM-5.3 仅 low/high/max)且不做
            # 映射,直接 400 拒绝;去掉该参数用模型默认档重试,不让脚本阶段失败
            self._log(
                f"  ⚠ 模型不接受思考深度 {effort},改用模型默认思考深度重试 …"
            )
            body.pop("reasoning")
            resp = self._post(body)
        ...
        if resp.status_code == 401:
            raise RuntimeError("OpenRouter API KEY 无效,请检查界面「设置」中的 OpenRouter API KEY")
        if resp.status_code == 402:
            raise RuntimeError("OpenRouter 余额不足,请前往 openrouter.ai 充值")
        if resp.status_code == 404:
            raise RuntimeError(
                f"模型不存在: {llm['model']},请检查界面「设置」中的编剧模型"
            )
    ...
    @staticmethod
    def _build_cuts(
        raw: dict, fallback_duration: int, min_group: int, max_group: int
    ) -> list[Cut]:
        """解析并钳制一个镜头组的分镜:单分镜 1~max_group 秒,组总长 min_group~max_group 秒。"""
        ...
        cuts = [
            Cut(
                ...
                duration=_clamp_duration(
                    c.get("duration"), fallback_duration, _MIN_CUT_SECONDS, max_group
                ),
            )
...
def _mentions_effort(resp: requests.Response) -> bool:
    """400 错误是否针对思考深度参数(reasoning / effort / thinking)。"""
    text = _error_message(resp).lower()
    return any(k in text for k in ("effort", "reasoning", "thinking"))
