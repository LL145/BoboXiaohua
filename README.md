...
④ ffmpeg                        → 按导演决定的硬切 / 交叉溶解拼接 + 首尾淡入淡出;
   │                              旁白默认由视频模型原生配音(画外音随画面一起生成),
   │                              选 TTS 合成时混入并自动闪避原生音轨;
...
- **旁白**:解说型影片的旁白默认由视频模型原生配画外音(与画面同步生成,声音
  自然;导演为全片固定一段声线描述,逐组随提示词送入以保持各组同一声线);
  界面「旁白」可改为 TTS 合成(Edge TTS 免费,不可用时自动改用 fal.ai 的
  MiniMax 中文 TTS,约 $0.10/千字)或关闭;
...
音效、并发数、TTS 旁白音色、转场等进阶参数在 `config.yaml` 中修改(点「更多设置
(配置文件)」可直接打开),一般保持默认即可。
